...
async function listItems(req, res) {
  try {
    ...
    const result = await db.query(
      `SELECT id, name, unit, minimum_stock, cost_per_unit, is_active, created_at
...
async function createItem(req, res) {
  const { name, unit, minimumStock, costPerUnit } = req.body;
  if (!name) return res.status(400).json({ error: 'Item name is required.' });
  try {
    const result = await db.query(
      `INSERT INTO inventory_items (name, unit, minimum_stock, cost_per_unit)
       VALUES ($1, $2, $3, $4) RETURNING id`,
      [name.trim(), unit || 'units', Number(minimumStock) || 0, costPerUnit ? Number(costPerUnit) : null]
    );
...
async function updateItem(req, res) {
  const { id } = req.params;
  const { name, unit, minimumStock, costPerUnit, isActive } = req.body;
  try {
    const sets = [];
    const values = [];
    let i = 1;
...
async function distribute(req, res) {
  ...
  for (const a of allocations) {
    if (!a.branchId || !a.quantity || Number(a.quantity) <= 0) {
      return res.status(400).json({ error: 'Every branch needs a valid quantity greater than zero.' });
    }
  }
  ...
  try {
    ...
    const itemRes = await client.query('SELECT * FROM inventory_items WHERE id = $1', [itemId]);
