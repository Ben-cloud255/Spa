...
const EMPTY_FORM = { name: '', unit: 'units', minimumStock: '', costPerUnit: '' };
...
export default function AdminInventoryPage() {
  ...
  const stockValue = stock.reduce((sum, s) => sum + Number(s.quantity) * Number(s.cost_per_unit || 0), 0);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white rounded-xl2 border border-forest-100 shadow-card p-5">
          ...
          <p className="text-xs text-forest-500/60 mt-1">TZS · priced items only</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-forest-50">
              {lowStockRows.map((s) => (
                <tr key={s.id}>
                  ...
                  <td className="px-4 py-3 text-clay font-medium">{s.quantity} {s.unit}</td>
                  <td className="px-4 py-3">{s.minimum_stock} {s.unit}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <form onSubmit={createItem} className="bg-white rounded-xl2 border border-forest-100 shadow-card p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-6 items-end">
        ...
        <div>
          <label className="block text-xs font-medium text-forest-600 mb-1.5">Unit</label>
          <input
            required
            value={form.unit}
            onChange={(e) => setForm({ ...form, unit: e.target.value })}
            placeholder="bottles, units…"
            className="rounded-lg border border-forest-200 px-3 py-2 text-sm w-full"
          />
        </div>
        <div>
              ...
              <tbody className="divide-y divide-forest-50">
                {displayedItems.map((it) => (
                  <tr key={it.id} className={it.is_active ? '' : 'opacity-50'}>
                    ...
                    <td className="px-4 py-3">{it.minimum_stock}</td>
                    <td className="px-4 py-3">{it.cost_per_unit ? `${money(it.cost_per_unit)} TZS` : '—'}</td>
              ...
              {displayedStock.map((s) => {
                ...
                return (
                  <tr key={s.id}>
                    <td className="px-4 py-3">{s.branch_name}</td>
                    <td className="px-4 py-3 font-medium">{s.item_name}</td>
                    <td className={`px-4 py-3 ${low ? 'text-clay font-medium' : ''}`}>{s.quantity} {s.unit}</td>
                  </tr>
                );
              })}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-forest-50">
              {displayedAllocations.map((a) => (
                <tr key={a.id}>
                  <td className="px-4 py-3 font-medium">{a.quantity} {a.unit} {a.item_name}</td>
                  <td className="px-4 py-3">{a.room_name}</td>
                  <td className="px-4 py-3">{a.branch_name}</td>
                  <td className="px-4 py-3">{a.quantity} {a.unit}</td>
                    ...
                    {a.status === 'completed' ? (
                      <>
                        ...
                        {Number(a.quantity_returned) > 0 && (
                          <div className="text-xs text-forest-500/50 mt-0.5">{a.quantity_returned} {a.unit} returned to stock</div>
                        )}
                      </>
                    ) : (
                      '—'
