...
export default function DistributeInventoryModal({
  ...
  const total = Object.values(selectedBranches).reduce((sum, qty) => sum + Math.round((Number(qty) || 0) * 100), 0) / 100;
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (total > available) {
      setError('The delivery exceeds the remaining quantity in the catalog.');
      return;
    }
    setSubmitting(true);
    try {
    ...
    <div className="fixed inset-0 bg-ink/40 backdrop-blur-sm flex items-center justify-center z-40 p-4 animate-modalBackdropIn">
      <div className="bg-white rounded-xl2 shadow-card w-full max-w-md p-6 animate-modalContentIn">
        ...
        <p className="text-sm text-forest-500/70 mb-5">
          Record the quantity sent to each branch. Your catalog quantity updates automatically.
        </p>
        ...
        {items.length === 0 ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {items.map((it) => (
                  <option key={it.id} value={it.id}>
                    {it.name} — Cost: {it.cost_per_unit == null ? '—' : Number(it.cost_per_unit)}
                  </option>
                ))}
              </select>
            </div>

            <p className="text-sm text-forest-600" aria-live="polite">Remaining quantity: {available} · Sending: {total} · Remaining after delivery: {Math.round((available - total) * 100) / 100}</p>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                type="submit"
                disabled={submitting || total > available || total <= 0}
