...
export default function NotificationBell() {
  ...
  useEffect(() => {
    if (unreadCount > prevCount.current) {
      setJustPulsed(true);
      const timeout = setTimeout(() => setJustPulsed(false), 500);
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="relative rounded-full h-10 w-10 flex items-center justify-center bg-white border border-forest-100 hover:bg-forest-50 active:scale-95 transition-all"
      >
        ...
        {unreadCount > 0 && (
          <span
            className={`absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-clay text-white text-[10px] font-semibold flex items-center justify-center ${
              justPulsed ? 'animate-pulseOnce' : ''
            }`}
          >
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-96 max-w-[90vw] bg-white rounded-xl2 shadow-card border border-forest-100 overflow-hidden z-30 animate-modalContentIn origin-top-right">
